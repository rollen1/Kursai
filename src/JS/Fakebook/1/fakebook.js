    ...
    fetch(peopleurl).then(parseJSON)
        .then(peopledata => {
            for (let i = 0; i < 6; i++) {
                feed.innerHTML += `
                <div class="card m-2">
                <img src="${peopledata.results[i].picture.large}" class="card-img-top" alt="...">
                <div class="card-body bg-dark text-light"
                  <h5 class="card-title">${peopledata.results[i].name.first + " " + peopledata.results[i].name.last}</h5>
                </div>
...
function createActivity() {
    fetch(activtyurl).then(parseJSON)
        .then(activitydata => {
